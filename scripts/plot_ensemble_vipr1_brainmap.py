...
from utils import get_centroids, index_structure
...
# Find the family that contains VIPR1
vip_family_name = gene_to_family.get('VIPR1', 'VIP/PACAP')
...
# 5. Fit Gating RandomForestClassifier
...
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
#                         ISOLATE VIPR1 (VIP/PACAP) CONTRIBUTION
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
print("Isolating VIPR1 (VIP/PACAP family) contribution...")
X_vip = np.zeros_like(X_fam)
X_vip[:, vip_family_idx] = X_fam[:, vip_family_idx]
X_vip_tensor = torch.tensor(X_vip, dtype=torch.float32)

# Predict VIPR1 contribution from each base model
u_gpls_vip = X_vip @ gpls.x_weights_[:, 0]
u_rf_vip = rf.predict(X_vip)
if pearsonr(rf.predict(X_fam), u_gpls)[0] < 0:
    u_rf_vip = -u_rf_vip

u_en_vip = en.predict(X_vip)
if pearsonr(en.predict(X_fam), u_gpls)[0] < 0:
    u_en_vip = -u_en_vip

with torch.no_grad():
    u_gcn_vip = model_gcn(X_vip_tensor, adj_fc).numpy().flatten()
if pearsonr(u_gcn, u_gpls)[0] < 0:
    u_gcn_vip = -u_gcn_vip

# Combine via gating weights
u_dynamic_vip = (pred_weights[:, 0] * u_gpls_vip +
                 pred_weights[:, 1] * u_rf_vip +
                 pred_weights[:, 2] * u_en_vip +
                 pred_weights[:, 3] * u_gcn_vip)

# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
#                         PLOT BRAIN MAP
# %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
print("Plotting cortical projection of ensemble VIPR1 contribution...")
...
# Slices the first 400 cortical regions from the full-brain 455-region prediction
vip_ctx_pred = u_dynamic_vip[:400]
...
for i in range(unique.shape[0]):
    plot_data = np.where(plot_data==unique[i], vip_ctx_pred[i], plot_data)
...
p.build(dpi=300, save_as='figs/VIPR1_ensemble_brainmap.pdf')
print("Successfully generated and saved VIPR1 ensemble brainmap to figs/VIPR1_ensemble_brainmap.pdf")
